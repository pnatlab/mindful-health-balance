...
ใน `สัญญาณวันนี้ 1/2` เปิด `มื้ออาหารวันนี้` แล้วเริ่มจากเลือกว่ามื้อนี้เป็นอาหารแบบไหน เช่น ผัด แกง หรืออาหารน้ำ จากนั้นค่อยเลือกสิ่งที่เห็นหรือรู้ว่าอยู่ในมื้อเท่านั้น ไม่จำเป็นต้องเดาสูตรหรือจำเครื่องปรุงให้ครบ จึงค่อยปรับปริมาณแบบ `น้อย / ปกติ / เยอะ / กำหนดเอง` และวิธีเตรียมเท่าที่จำได้ จะตั้ง `ชื่อมื้อ / ชื่ออาหาร` แบบธรรมชาติเพื่อช่วยจำมื้อนี้ก็ได้ แต่เป็นข้อมูลบริบทของผู้ใช้ ไม่ได้หมายความว่ารายการส่วนประกอบครบหรือเป็นการประเมินโภชนาการ การเก็บหนึ่งมื้อไม่บังคับชื่อมื้อหรือเวลา และเพิ่มได้หลายมื้อในวันเดียวกัน
...
ตัวช่วยจากรูปเป็น prototype แบบ optional สำหรับ PNG, JPEG, WebP, HEIC และ HEIF โดยใช้โมเดลในเครื่องนี้เท่านั้น หากเป็นรูปจากโทรศัพท์ขนาดใหญ่ ให้เลือก `เตรียมรูปสำหรับ AI`: ระบบจะเปิดหน้าแยกเพื่อเตรียม JPEG ชั่วคราวในเครื่อง ให้ตรวจรูปก่อน แล้วส่งกลับมาตัวช่วยจากรูปโดยไม่เก็บไว้กับมื้อ หลังเลือกรูป ระบบจะแสดงสิ่งที่มองเห็นให้ตรวจก่อน ผู้ใช้เลือกละเว้นหรือแก้ mapping ที่ยังคลุมเครือได้ แล้วจึงกด `ใช้รายการที่เลือกกับมื้อนี้` เพื่อเติมเฉพาะ Meal Type/องค์ประกอบที่ยอมรับลงใน draft เดิม การกดนี้ยังไม่บันทึกมื้อ ไม่ตั้ง `named_dish_id` และไม่สร้างค่าโซเดียมจากภาพ; เมนูอ้างอิงยังต้องยืนยันแยกอีกครั้งตามเดิม หากโมเดลไม่พร้อมยังประกอบมื้อเองต่อได้ตามปกติ

ที่หน้า `Log` ให้ขยาย `ร่องรอยจากตาของ MHB` เพื่อดูคำที่ตัวช่วยจากรูปเคยสังเกตไว้ในเครื่อง พร้อมจำนวนครั้งและสถานะการ map หน้านี้มีไว้ดูช่องว่างของฐานอาหารเท่านั้น ไม่เพิ่ม Food Reference หรือเปลี่ยนมื้อให้อัตโนมัติ

สำหรับการเตรียมรูปและตัวช่วยจากรูปในเครื่องบน macOS ให้ build หนึ่งครั้งด้วย `./tools/build_mhb_macos_app.sh` แล้วดับเบิลคลิก `dist/Mindful Health Balance.app` ระบบจะเปิดที่ canonical loopback origin `http://127.0.0.1:4173` เท่านั้น แอปนี้เป็น launcher ส่วนตัวในเครื่อง ไม่มี Developer ID หรือ App Store signature; ถ้ามี service อื่นใช้พอร์ตนี้อยู่ MHB จะหยุดพร้อมข้อความชัดเจน แทนการสลับไป browser storage คนละชุดโดยเงียบ ๆ ให้ปิด service นั้นเมื่อทราบว่าเป็นอะไร แล้วลองเปิด MHB อีกครั้ง `Start Mindful Health Balance.command` ยังคงเป็นทางเลือกสำรองและใช้ตรวจปัญหาได้ หากเปิด `index.html` ตรง ๆ ยังประกอบมื้อเองได้ตามปกติ แต่ปุ่มรูปจะแจ้งว่าต้องใช้ Local Launcher ทางเลือกแบบ manual คือ `python3 -m http.server 4173 --bind 127.0.0.1` แล้วเปิด `http://127.0.0.1:4173/index.html`
...
หาก MHB อ่าน Daily Log ใน browser นี้ไม่ได้ ระบบจะแสดงข้อความกู้คืนแทนการตีความว่าเป็น 0 logs และจะพัก Save, Import, Clear, Restore และ Export เพื่อรักษาค่าเดิมไว้ก่อน ให้กด `สำรองข้อมูลดิบไว้ในเครื่อง` แล้วใน session เดียวกันจึงเลือก Master Excel ที่เชื่อถือได้เพื่อกู้คืนอย่างชัดเจน ระบบไม่ upload หรือ reset ข้อมูลแบบเงียบ ๆ
