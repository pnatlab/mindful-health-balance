...
照片助手是一个可选的限定 prototype，仅支持 PNG、JPEG 与 WebP，并只使用这台设备上的模型。选择照片后，先核对模型观察到的内容；可以省略建议，并为含糊项目选择正确映射，再点“把所选内容用于这一餐”。这只会把已接受的餐食类型/组成加入现有草稿，不会保存餐食、设置 `named_dish_id`，也不会从图片生成钠证据；参考菜品仍需另行确认。本地模型不可用时，仍可照常手动组合餐食。
