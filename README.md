...
For local Image Prep and Vision, double-click `Start Mindful Health Balance.command` on macOS. It opens MHB at `http://127.0.0.1` without exposing the server to the network. Opening `index.html` directly still supports manual MHB, but shows a reminder that photo capabilities need the Local Launcher. Manual fallback: `python3 -m http.server 4173 --bind 127.0.0.1`, then open `http://127.0.0.1:4173/index.html`.
